...
dataInp <- dataInp[[c("BB06","BB12","BB14")]]
####
...
server <- function(input, output) {
    ...
    for(i in 1:ntabs){
      Tabs[[i]] <- tabItem(tabName = tabnames[i],
                           fluidRow( ## make the same as in "population corridors"
                             ...
                             column(2, numericInput(timeThinnames[i],"Thin track to X mins",min=0,max=60, value=0, step=1),
                             ...
                             column(1,actionBttn(updateButton[i], label="Update!", style="fill", color="success",icon=icon("redo"),size="md"))
                           ),
                           withSpinner(plotOutput(plotnames[i],dblclick = paste0(plotnames[i],"_dblclick"), brush = brushOpts(id = paste0(plotnames[i],"_brush"),resetOnNew = TRUE), height = "80vh"), type=5, size=1.5,color= "#28b78d") ## color the same as update button
      )
    }
  ...
  observe({
    RVtab$indv <- namesCorresp$nameInd[namesCorresp$tabIndv==input$sidebarMenu]
  })
  ## get input values for each tab after hiting update button
  RVupdate <- reactiveValues()
  observeEvent({input[[paste0(input$sidebarMenu, '_updateButton')]]},{
    RVupdate$thintime <- input[[paste0(input$sidebarMenu, '_timeThin')]]
    RVupdate$speedProp <- input[[paste0(input$sidebarMenu, '_speedProp')]]
    RVupdate$circProp <- input[[paste0(input$sidebarMenu, '_circProp')]]
    RVupdate$clustDist <- input[[paste0(input$sidebarMenu, '_clustDist')]]
  })
  
  for(i in 1:ntabs){
    output[[plotnames[i]]] <- renderPlot({
      dataSubInd <-  dataInp[[RVtab$indv]]
      
      if(!input[[paste0(input$sidebarMenu, '_updateButton')]]){ ## default plot
