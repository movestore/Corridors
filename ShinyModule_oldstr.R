...
## TODO
# give all tracks the same color and transparancy
# add the selected corridors to the output data
# think about wht to to woth the locID column, or how to go about it...
...
shinyModule <- function(input, output, session, data) {
  ...
  # data_out <- reactive(data) ## NOT SURE IF THIS IS NEEDED HERE..., probbaly not if corridros should be added
    ...
    for(i in 1:ntabs){
      Tabs[[i]] <- tabItem(tabName = tabnames[i],
                           fluidRow( ## make the same as in "population corridors"
                             ...
                             column(1, numericInput(ns(clustDistnames[i]),"Cluster radius (m)",value=300),
                             ...
                             column(1, numericInput(ns(clustNbnames[i]),"Segment number",value=3),
                                    bsTooltip(id=ns(clustDistnames[i]), title="Minimum number of segments that will define a cluster. Clusters with fewer segments will be excluded", placement = "bottom", trigger = "hover", options = list(container = "body"))
                             ),
  ...
  for(i in 1:ntabs){
    ...
    if(i==1){
      ...
      output[[plotnames[i]]] <- renderLeaflet({
        ...
        if(!input[[paste0(input$sidebarMenuUI, '_updateButton')]]){ ## default plot
          ...
          color: black;
            ...
            ### calculating clusters to define which segments belong "to the same corridor"
            # clusterRadius <- 200 ## RVupdate$clustDist ## ADJUST THIS VALUE!!! - diameter of the cluster, i.e. width of the corridor
            # nrKeep <- 2 ## RVupdate$clustNb ## ADJUST THIS VALUE!!! - change to number of segments that corridor should have within a cluster
            ...
            midCrPts$clusterID <- cutree(hc, h=RVupdate$clustDist/2) # h=RVupdate$clustDist
            ...
            midCrPts <- midCrPts[midCrPts$nbCorrInClust>RVupdate$clustNb,]
            ...
            map1 <- leaflet(corridorCalc) %>% addTiles()%>%
              addProviderTiles("Esri.WorldTopoMap", group = "TopoMap") %>%
              addProviderTiles("Esri.WorldImagery", group = "Aerial") %>% 
              addPolylines(lng = coordinates(corridorCalc)[,1],lat = coordinates(corridorCalc)[,2],weight=2, opacity=0.5, layerId=namesIndiv(corridorCalc),group ="track")
            # for(n in corrDFr){ ## to plo all corridors at once
            #   map1 <- map1 %>%
            ...
            map1 %>% 
             # addCircles(data=centClust, lng=~x, lat=~y, group=~clusterID, radius=RVupdate$clustDist/2, color="gold") %>% #radius=RVupdate$clustDist
              ...
              addLayersControl(
                baseGroups = c("TopoMap","Aerial"),
                overlayGroups = c("track",paste0("potential corridors gr:",selClusterIDs)),#,centClust$clusterID),
      ...
      ## this is a way to get the layers that are selected
      ...
      #   selClusts <- as.numeric(gsub("[^\\d]+", "", selected_groups, perl=TRUE))
      #   selLocIDs <- indDF$LocID[indDF$clusterID%in%selClusts]
      #   print(selClusts)
      # })
